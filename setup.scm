...
(define (populate-weapons rooms)
  ...
  (create-weapon 'large-stick (pick-random rooms) 3);*
  (create-weapon 'moderate-stick (pick-random rooms) 2);*
  (create-weapon 'small-stick (pick-random rooms) 1);* I addded the starred ones.
  'populated-weapons) 	   	 	     
...
(define (populate-players rooms)
  ...
  (create-troll 'cici (car (myfilter (lambda (x) (eq? (ask x 'name) 'cici-bufe))
                                     rooms))
                3 1)
  (create-violent-person 'violent-george (pick-random rooms) 2 2 3);*
  (create-violent-person 'violent-stephano (pick-random rooms) 2 3 3);*
  (create-violent-person 'violent-micheal (pick-random rooms) 1 2 3);* I added the ones with stars
  (create-canadian 'canadian-justin (pick-random rooms) 2 2)
  (create-canadian 'canadian-william (pick-random rooms) 1 2)
  (create-canadian 'canadian-charles (pick-random rooms) 2 1)
  'populated-players) 	   	 	     
...
(define (setup name) 	   	 	     
  ...
  (let ((rooms (create-world)))
    ...
    (populate-weapons rooms) ;uncommented	   	 	     
    (populate-players rooms) 	   	 	     
    (populate-bombs rooms) ;added
